...
const linksUrl = new URL("secrets/links.enc.yaml", root);
...
async function readLinks() {
  try {
    const plain = execFileSync("sops", ["--decrypt", fileURLToPath(linksUrl)], {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
    });
    return parseYaml(plain);
  } catch (error) {
    console.warn(`Could not read secrets/links.enc.yaml (${error.message.split("\n")[0]})`);
  }

  console.warn("Building from links.example.json instead");
...
const banner = "<!-- Generated by scripts/build.mjs. Edit src/ and secrets/links.enc.yaml instead. -->\n";

const { version } = JSON.parse(await readFile(new URL("package.json", root), "utf8"));
const buildDate = new Date().toISOString().slice(0, 10);

page = page.replaceAll("{{version}}", version).replaceAll("{{date}}", buildDate);

await mkdir(new URL("dist/", root), { recursive: true });
await writeFile(outUrl, banner + page, "utf8");

console.log(`Wrote ${fileURLToPath(outUrl)} v${version} with ${links.sections.length} sections`);
